...
## This is here for folks who don't reset the R session.
rm(list = ls())

library(knitr)
library(tidyverse)
data_file <- "./data/new-york-city-airbnb-open-data.zip"
abnb <- read_csv(data_file)
```
...
If you are coming back in, be sure to run setup before attempting to work on the tasks below.
...
## Task 04. Number of Airbnb hosts per `neighbourhood_group`. 

- Question: Which neighborhood has the most hosts?
- Answer:

Task: Draw a bar plot showing the number of Airbnb hosts per `neighbourhood_group` to answer the question.
...
- Question: How many hosts are in Queens?
- Answer: 
...
## Task 13. Is this a large enough sample to be useful?
